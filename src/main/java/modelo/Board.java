...
public class Board {
	...
	public Boolean makeMove(Move move) {
		...
		for (Move x : pieceFrom.getMoves()) {
			...
			if (x.equals(move)) {
				...
				incHalfMoves(); //DRAW BY FIFTY MOVES RULE
				...
				gameStatus = manageChecks(pos_to, gameStatus);
				colorToMove ^= 1;
	...
	private int manageChecks(int pos_to, int gameStatus) {	
		if (!in_check) {
			var check = lookForChecks(pos_to, 1-colorToMove);
			if (check) {
				in_check = true;
				givingCheck = pos_to;
			}
		} else {
			if (lookForChecks(givingCheck, colorToMove)) {
	...
	private Boolean lookForChecks(int pos, int colorOtherKing) {
		var pieceFrom = getPiece(pos);
		if (pieceFrom == null) return false;
		
		pieceFrom.updateMoves(this);
		var king_pos = (colorOtherKing == 0) ? whiteKingPos : blackKingPos;
		
		for (Move m : pieceFrom.getMoves()) {
			var pos_to = m.getPos_to();
			if (pos_to == king_pos) {
				return true;
			}
